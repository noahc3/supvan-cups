...
mod imp {
    ...
    impl SppPipe for BlePipe {
        ...
        async fn send_raw_frame(&self, _frame: &[u8]) -> Result<Option<Vec<u8>>> {
            // The E-series raw setup frames (0xD0/0xB0) are only exercised over
            // Classic-BT RFCOMM (the E10pro is RFCOMM-only). The BLE GATT pipe
            // has no raw-frame concept, so this is unsupported.
            Err(Error::Ble(
                "E-series raw command frames are not supported over BLE".into(),
            ))
        }
    }
}
